/* Stock Wine 11.15 windef.h defines max/min as macros unless NOMINMAX is set, */
/*  which breaks the std::max in collapse_path. */
#define NOMINMAX
#include "ntstatus.h"
...
#ifdef __APPLE__
/* The FEX wine loader is hardened without allow-dyld-environment-variables, so
 * dyld drops the insert list that would carry the overlay renderer and the
 * notproton shim. Load them by hand instead. */
static void load_overlay_libraries( void )
{
    const char *inserts = getenv( "STEAM_DYLD_INSERT_LIBRARIES" );
    const char *shim = getenv( "NOTPROTON_OVERLAY_SHIM" );
    void *handle, *shim_handle = NULL;
    void (*install)( void );
    char *list, *cursor, *path;

    if (!inserts || !*inserts) return;

    if (shim && *shim && (handle = dlopen( shim, RTLD_NOW | RTLD_NOLOAD )))
    {
        TRACE( "overlay shim already inserted, its interposes are live\n" );
        dlclose( handle );
        return;
    }

    if (shim && *shim && !(shim_handle = dlopen( shim, RTLD_NOW )))
        WARN( "unable to load overlay shim %s: %s\n", debugstr_a(shim), dlerror() );

    if (!(list = strdup( inserts ))) return;
    cursor = list;
    while ((path = strsep( &cursor, ":" )))
    {
        if (!*path) continue;
        if (!dlopen( path, RTLD_NOW )) WARN( "unable to load %s: %s\n", debugstr_a(path), dlerror() );
        else TRACE( "Loaded %s\n", debugstr_a(path) );
    }
    free( list );

    if (shim_handle && (install = (void (*)( void ))dlsym( shim_handle, "np_overlay_shim_install" )))
        install();
}
#endif /* __APPLE__ */
...
#define LOAD_FUNC_OPTIONAL( x )                               \
    p_##x = (decltype(p_##x))dlsym( steamclient, #x )

    LOAD_FUNC_OPTIONAL( Steam_IsKnownInterface );
    LOAD_FUNC_OPTIONAL( Steam_NotifyMissingInterface );
...
{
    params->_ret = p_Steam_IsKnownInterface ? p_Steam_IsKnownInterface( params->version ) : true;
    return 0;
}
